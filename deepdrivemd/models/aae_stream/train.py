...
def build_model(cfg: Point3dAAEConfig):
    ...
    with Timer("ml_aae"):
        ...
        if(not cfg.reinit):
            checkpoint = torch.load(cfg.checkpoint_dir + "/best.pt", map_location="cpu")
...
def main(cfg: Point3dAAEConfig):
    ...
    for i in itertools.count(0):
        ...
        if "aae" not in locals():
            aae, device = build_model(cfg)
        ...
        with Timer("ml_train"):
            try:
                disc_optimizer = get_torch_optimizer(
                    ...
                    aae.discriminator.parameters(),
                )
                ae_optimizer = get_torch_optimizer(
                    cfg.ae_optimizer.name,
                    cfg.ae_optimizer.hparams,
                    itertools.chain(aae.encoder.parameters(), aae.decoder.parameters()),
                )

                train_model(
                    aae, ae_optimizer, disc_optimizer, train_loader, valid_loader, device, cfg
                )
        ...
        best_model = f"{cfg.checkpoint_dir}/best.pt"
        if os.path.exists(best_model):
            subprocess.getstatusoutput(
                f"mv {best_model} {cfg.published_model_dir}/"
            )
